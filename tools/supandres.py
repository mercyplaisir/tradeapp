...
def sup_res(df)->Dict[str,List[float]]:
    """ generating support and resistance"""
    ...
    # Creating a list and feeding it the identified support and resistance levels
    #  via the Support and Resistance functions
    levels = []
    level_types = []
    ...
    # (i.e. if distance to the next level is less than the average candle size
    #  for any given day - this will give a rough estimate on volatility)
    mean = np.mean(df['high'] - df['low'])

    # This function, given a price value, returns True or False depending
    #  on if it is too near to some previously discovered key level.
    def distance_from_mean(level):
        return np.sum([abs(level - y) < mean for y in levels]) == 0

    # Optimizing the analysis by adjusting the data and eliminating 
    # the noise from volatility that is causing multiple levels to show/overlapp
    levels = []
    level_types = []
    supports_levels = []
    resistance_levels = []
    ...
    return {"support":supports_levels,"resistance":resistance_levels}
